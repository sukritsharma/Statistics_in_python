...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 24 22:47:26 2021
...
cutoff_options=[['0','saved_files/Grade_map_1/RFECV_RF_without_threshold/'],['threshold_data[metabolite_name].min()','saved_files/Grade_map_1/RFECV_RF_hotspot_min_filter/'],['threshold_data[metabolite_name].max()','saved_files/Grade_map_1/RFECV_RF_hotspot_max_filter/'],['threshold_data[metabolite_name].mean()','saved_files/Grade_map_1/RFECV_RF_hotspot_mean_filter/']]
...
for opt in cutoff_options:
    ...
    # cfeature selection
    rfecv = RFECV(estimator=RandomForestClassifier(n_estimators = 10, random_state = 1), step=1,n_jobs=-1, scoring='roc_auc',cv=5, verbose=2)
    ...
    imp_feature_df["features"]=features_list[rfecv.support_]
    ...
    # plt.xticks(range(0, len(rfecv.estimator_.feature_importances_) ), features_list[rfecv.support_], rotation='vertical')
    ...
    for pat in patients:
        ...
        plt.xticks(range(1, len(rfecv.grid_scores_) + 1), features_list, rotation='vertical')
        plt.show()
        feature_plot_name=save_data_path+pat_code + '_feature_plot.png'
        ...
        n_scores = cross_val_score(rf, rfecv.transform(X), y, scoring='roc_auc', cv=4, n_jobs=-1, error_score='raise')
